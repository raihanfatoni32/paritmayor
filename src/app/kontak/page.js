...
export default function KontakPage() {
  ...
  return (
    <div className="flex flex-col">
      ...
      {/* ════════════════════════════════════════════════════
          4. FORMULIR PENGADUAN DIGITAL — Full form interaktif
          ════════════════════════════════════════════════════ */}
      <section
        id="formulir-pengaduan"
        aria-labelledby="heading-form-pengaduan"
        className="bg-white py-12 md:py-16"
      >
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            <h2 id="heading-form-pengaduan" className="text-2xl sm:text-3xl font-extrabold text-gray-900 mb-2">
              Formulir Pengaduan Digital
            </h2>
            <p className="text-gray-500 text-sm">
              Laporan Anda akan diteruskan langsung ke admin kelurahan melalui WhatsApp.
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-md p-6 sm:p-8">
              ...
              <div className="mb-6 p-4 rounded-xl bg-green-50 border border-green-200 text-green-800 text-sm flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs mt-0.5">
                    Laporan Anda telah diarahkan ke WhatsApp admin kelurahan. Tim kami akan segera menindaklanjuti.
                  </p>
                </div>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {/* Nama */}
                <div>
                  <label htmlFor="kontak-input-nama" className="block text-xs font-semibold text-gray-700 mb-1.5">
                    Nama Lengkap <span className="text-red-500" aria-hidden="true">*</span>
                  </label>
                  <input
                    id="kontak-input-nama"
                    type="text"
                    name="nama"
                    value={form.nama}
                  ...
                  />
                  {errors.nama && (
                    <p role="alert" className="text-red-500 text-xs mt-1">{errors.nama}</p>
                  )}
                </div>

                {/* NIK */}
                <div>
                  <label htmlFor="kontak-input-nik" className="block text-xs font-semibold text-gray-700 mb-1.5">
                    NIK <span className="text-red-500" aria-hidden="true">*</span>
                  </label>
                  <input
                    id="kontak-input-nik"
                    type="text"
                  ...
                  />
                  {errors.nik && (
                    <p role="alert" className="text-red-500 text-xs mt-1">{errors.nik}</p>
                  )}
                </div>
              </div>
              ...
              {/* Domisili RT/RW */}
              <div>
                <label htmlFor="kontak-select-domisili" className="block text-xs font-semibold text-gray-700 mb-1.5">
                  Domisili (RT/RW) <span className="text-red-500" aria-hidden="true">*</span>
                </label>
                <select
                  id="kontak-select-domisili"
                  name="domisili"
                  value={form.domisili}
                  onChange={handleChange}
                  className={`w-full rounded-xl border px-4 py-3 text-sm bg-white outline-none transition-all focus:ring-2 appearance-none ${
                    errors.domisili
                      ? "border-red-400 focus:ring-red-100 bg-red-50"
                      : "border-gray-200 focus:border-blue-400 focus:ring-blue-50"
                  }`}
                ...
                {errors.domisili && (
                  <p role="alert" className="text-red-500 text-xs mt-1">{errors.domisili}</p>
                )}
              </div>

              {/* Detail Laporan */}
              <div>
                <label htmlFor="kontak-textarea-laporan" className="block text-xs font-semibold text-gray-700 mb-1.5">
                  Detail Laporan / Aduan <span className="text-red-500" aria-hidden="true">*</span>
                </label>
                <textarea
                  id="kontak-textarea-laporan"
                  name="laporan"
                  value={form.laporan}
                  onChange={handleChange}
                  placeholder="Tuliskan detail laporan Anda dengan jelas: lokasi kejadian, waktu, dan keterangan lainnya..."
                  rows={5}
                  className={`w-full rounded-xl border px-4 py-3 text-sm resize-none outline-none transition-all focus:ring-2 ${
                    errors.laporan
                      ? "border-red-400 focus:ring-red-100 bg-red-50"
                      : "border-gray-200 focus:border-blue-400 focus:ring-blue-50"
                  }`}
                />
                {errors.laporan && (
                  <p role="alert" className="text-red-500 text-xs mt-1">{errors.laporan}</p>
                )}
                <p className="text-gray-400 text-xs mt-1 text-right">
                  {form.laporan.length}/500 karakter
                </p>
              </div>
              ...
              <button
                type="submit"
                id="btn-kirim-laporan-wa"
                className="w-full flex items-center justify-center gap-3 text-white font-bold text-sm py-4 rounded-xl shadow-lg transition-all duration-200 hover:opacity-90 hover:-translate-y-0.5 hover:shadow-xl"
              ...
              <p className="text-gray-400 text-xs text-center">
                Dengan mengirim formulir ini, Anda menyetujui bahwa data yang diberikan akan digunakan untuk proses penanganan pengaduan.
              </p>
            </form>
          </div>
        </div>
